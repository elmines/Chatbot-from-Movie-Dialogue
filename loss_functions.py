...
def min_affective_dissonance(train_logits,targets,target_lengths,enc_embed_input,dec_embed_input):
    lamba = tf.Variable(0.5, name="lamba")
    lamba_inv=tf.Variable(4.0,name="lamba_inv")
...
def max_affective_dissonance(train_logits,targets,target_lengths,enc_embed_input,dec_embed_input):
    lamba = tf.Variable(0.5, name="lamba")
    lamba_inv=tf.Variable(4.0,name="lamba_inv")
    predicted_prob = tf.reduce_max(dec_embed_input)
...
def max_affective_content(train_logits,targets,target_lengths,dec_embed_input):
    lamba = tf.Variable(0.5, name="lamba")
    lamba_inv=tf.Variable(4.0,name="lamba_inv")
    predicted_prob = tf.reduce_max(dec_embed_input)
    eta =[5,1,5]  
